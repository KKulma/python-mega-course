...
def command_search():
    screen.delete(0, END)
    for row in database.search(title=e_title_value.get(), author=e_author_value.get(), year=e_year_value.get(), isbn=e_isbn_value.get()):
        screen.insert(END, row)

def command_add():
    database.insert(title=e_title_value.get(), author=e_author_value.get(), year=e_year_value.get(), isbn=e_isbn_value.get())
    screen.delete(0, END)
...
def command_update():
    database.update(selected_tuple[0], e_title_value.get(), e_author_value.get(), e_year_value.get(), e_isbn_value.get())
